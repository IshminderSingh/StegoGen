...
class StegoGenApp(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        self.analysis_frame = ttk.Frame(self.notebook, padding=12)

        self.notebook.add(self.encode_frame, text="   Encode (Hide Data)   ")
        self.notebook.add(self.decode_frame, text="   Decode (Extract Data)   ")
        self.notebook.add(self.analysis_frame, text="   Image Quality Analysis   ")
    ...
    def _build_analysis_tab(self) -> None:
        c_box = ttk.LabelFrame(self.analysis_frame, text=" 1. Original Cover Image ", padding=8)
        c_box.pack(fill="x", pady=4)
        self.analysis_cover_var = tk.StringVar()
        ttk.Entry(c_box, textvariable=self.analysis_cover_var, width=55).pack(side="left", fill="x", expand=True, padx=4)
        ttk.Button(c_box, text="Browse Cover", command=self._browse_analysis_cover).pack(side="left", padx=4)

        s_box = ttk.LabelFrame(self.analysis_frame, text=" 2. Stego Image ", padding=8)
        s_box.pack(fill="x", pady=4)
        self.analysis_stego_var = tk.StringVar()
        ttk.Entry(s_box, textvariable=self.analysis_stego_var, width=55).pack(side="left", fill="x", expand=True, padx=4)
        ttk.Button(s_box, text="Browse Stego", command=self._browse_analysis_stego).pack(side="left", padx=4)

        btn_row = ttk.Frame(self.analysis_frame)
        btn_row.pack(fill="x", pady=8)
        ttk.Button(btn_row, text="Run Quality Analysis (MSE / PSNR / SSIM)", command=self._run_analysis).pack(side="left", padx=4)

        res_box = ttk.LabelFrame(self.analysis_frame, text=" Analysis Report ", padding=8)
        res_box.pack(fill="both", expand=True, pady=4)
        self.analysis_report_box = tk.Text(res_box, height=12, wrap="word", font=("Consolas", 10))
        self.analysis_report_box.pack(fill="both", expand=True, pady=4)

    def _browse_analysis_cover(self) -> None:
        p = filedialog.askopenfilename(title="Select Original Cover Image")
        if p:
            self.analysis_cover_var.set(p)

    def _browse_analysis_stego(self) -> None:
        p = filedialog.askopenfilename(title="Select Stego Image")
        if p:
            self.analysis_stego_var.set(p)

    def _run_analysis(self) -> None:
        c_path = self.analysis_cover_var.get().strip()
        s_path = self.analysis_stego_var.get().strip()

        if not c_path or not Path(c_path).is_file():
            messagebox.showerror("Error", "Please select a valid cover image.")
            return
        if not s_path or not Path(s_path).is_file():
            messagebox.showerror("Error", "Please select a valid stego image.")
            return

        try:
            metrics = compare_images(c_path, s_path)
            self.analysis_report_box.delete("1.0", "end")
            self.analysis_report_box.insert("1.0", metrics.summary())
        except Exception as exc:
